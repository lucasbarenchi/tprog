...
<!--             Page content  -->
            <div id='page-content'>
            	<div class="row">
	            	...
	            	<% if(!playlist.isPrivate() || playlist.getOwner().getOwner().equals(request.getSession().getAttribute("usuario_logueado"))) {%>	
	            		<h3><%=playlist.getName() %></h3>
	            		
		            		<% String categories = playlist.getCategories().stream().collect(Collectors.joining(" | ")); %>
		            		<p class="text-secondary"><%= categories %></p>
		            </div>
		            <% if (playlist.getOwner().getOwner().equals(request.getSession().getAttribute("usuario_logueado")) && playlist.isPrivate()){ %>
			            <div class="col-md-2 pl-5">
			            	<form method="post" action="/makePlaylistPublic?id=<%=playlist.getPlaylistId()%>">
			            	<button class="btn btn-danger navbar-btn" style="align:right">Hacer pública</button>
			            	</form>
			            </div>
			            <%
			            	} else if (playlist.getOwner().getOwner().equals(request.getSession().getAttribute("usuario_logueado"))){
			            %>
			            <div class="col-md-2 pl-5">
			            	<form method="post" action="/makePlaylistPrivate?id=<%=playlist.getPlaylistId()%>">
			            	<button class="btn btn-danger navbar-btn" style="align:right">Hacer privada</button>
			            	</form>
			            </div>
			            <% }%>
	            </div>
