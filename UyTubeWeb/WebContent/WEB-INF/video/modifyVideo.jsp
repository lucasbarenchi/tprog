...
<%@page import="java.util.ArrayList, java.util.List, com.main.server.service.CategoryDT, com.main.server.service.VideoDT"%>
<!DOCTYPE html>
<html>
...
<!--             Page content  -->
            <div id='page-content'>
	            ...
	            <% VideoDT video = (VideoDT) request.getAttribute("video"); %>
				<form method="post" action="/modify_video?id=<%=video.getVideoId()%>">
					<div class="form-row">
				    	<div class="form-group col-md-4">
				      		<label for="videoName" style="color:white">Nombre del video</label>
				      		<input type="text" name="video_name" value="<%= video.getTitle() %>" class="form-control" id="inputName" placeholder="Top 10 videos más graciosos" required>
				    	</div>
					    <div class="form-group col-md-4">
					    	<label for="duration" style="color:white">Duración (en segundos)</label>
					    	<input type="number" name="length" value="<%= video.getLength() %>" class="form-control" id="inputLength" placeholder="45" required>
					    </div>
					    <div class="form-group col-md-4">
					    	<label for="publicDate" style="color:white">Fecha de publicación</label>
					    	<input type="date" name="upload_date" value="<%= video.getUploadDate().toString() %>" class="form-control" id="inputPublicDate" placeholder="aaaa" required>
					    </div>
				  	</div>
					...
					<div class="form-row">
				    	<div class="form-group col">
				     		...
				     		<input type="text" name="url" value="<%= video.getUrl() %>" class="form-control" id="inputURL" placeholder="https://www.youtube.com/watch?v=vu_mfmNHtYM" required>
				    	</div>
				  	</div>
				  
				  	<div class="form-group">
				    	<label for="description">Descripción</label>
				    	<textarea name="description" class="form-control" id="inputDesc" rows="3" required><%= video.getDescription() %></textarea>
				  	</div>
				  
				  	<div class="form-group">
				    	<label for="category">Categoría (opcional)</label>
				    	<select name="category" class="form-control" id="inputCategory">
				        <option hidden> <%= video.getCategory() != null ? video.getCategory() : "-"  %> </option>
				  	...
				  	<div class="form-row pl-1">
				  		<div class="form-check pr-3">
							<input class="form-check-input" type="radio" name="privacy" id="publicRadio" value="public" <%= !video.isPrivate() ? "checked" : "" %>>
							<label class="form-check-label" for="publicRadio">Público</label>
							</div>
						<div class="form-check">
							<input class="form-check-input" type="radio" name="privacy" id="privateRadio" value="private" <%= video.isPrivate() ? "checked" : "" %>>
							<label class="form-check-label" for="privateRadio">Privado</label>
						</div>
						</div>
				  	<div>
				 		<input type="submit" value="Publicar" class="btn btn-danger mt-3" >Publicar</input>
				  	</div>
				</form>
            </div>
        </div>
